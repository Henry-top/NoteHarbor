...
墨岛笔记是一款本地优先的 Markdown 个人笔记应用。笔记始终保存为普通 `.md` 文件，应用只在系统数据目录维护搜索索引、偏好设置与历史快照。
...
新建笔记使用 YAML 保存稳定 ID、时间和标签；导入的既有 Markdown 不会仅因打开或索引而被改写。跨资料库搜索不会生成专用跨库链接，确保每个资料库可以独立移动和使用。
